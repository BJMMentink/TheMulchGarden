...
let boardSocial = { groups: [], posts: [], replies: [], ideas: [] };
...
function ensureBoardState() {
  ...
  if (!Array.isArray(state.board.ideas)) state.board.ideas = [];
  return state.board;
}
...
function boardProjectsForUser(board) {
  const ownProjects = board.projects.map((project) => ({ ...project, ownerId: project.ownerId || currentUser.id, ownerUsername: project.ownerUsername || currentUser.username }));
  const ownIds = new Set(ownProjects.map((project) => project.id));
...
const BOARD_SIDEBAR_SECTIONS = ['pinned', 'folders', 'recent'];
...
function boardSidebarSectionOrder() {
  try {
    ...
    if (Array.isArray(stored) && stored.join(',') === 'pinned,recent,folders') return [...BOARD_SIDEBAR_SECTIONS];
...
function boardFeedPage() {
  ...
  const sidebarButtonLabel = boardFeedSidebarOpen ? 'Collapse groups sidebar' : 'Expand groups sidebar';
  const sidebarButtonText = boardFeedSidebarOpen ? 'Hide' : 'Show';
  return `<section class="board-page board-social-page"><div class="board-social-layout${sidebarState}"><aside class="board-social-sidebar${boardFeedSidebarOpen ? '' : ' is-collapsed'}"><button type="button" class="board-sidebar-toggle" data-board-toggle-sidebar aria-expanded="${boardFeedSidebarOpen}" aria-controls="board-social-sidebar-content" aria-label="${sidebarButtonLabel}"><span class="board-sidebar-toggle-mark" aria-hidden="true">${boardFeedSidebarOpen ? '‹' : '›'}</span><span class="board-sidebar-toggle-label">${sidebarButtonText} groups</span></button><div class="board-social-sidebar-content" id="board-social-sidebar-content"><div class="board-social-sidebar-top"><span class="editorial-kicker">Project board</span><h2>People, projects, in progress.</h2><p>Share a useful step with the groups who can help move it forward.</p></div><div class="board-social-section board-workspace-links"><span class="board-social-label">Workspace</span><button type="button" class="board-workspace-link is-active" data-board-mode="feed"><span>Feed</span><small>Updates and replies</small></button><button type="button" class="board-workspace-link" data-board-mode="ideas"><span>Idea Lab</span><small>Shape rough ideas</small></button><button type="button" class="board-workspace-link" data-board-mode="library"><span>Projects</span><small>Repos and folders</small></button></div><div class="board-social-section"><button type="button" class="board-all-groups ${!selectedGroup ? 'is-active' : ''}" data-board-group="all">All group posts <strong>${posts.length}</strong></button><span class="board-social-label">Your groups</span>${memberGroups.length ? memberGroups.map(groupButton).join('') : '<p class="board-sidebar-empty">Create or join a group to shape your feed.</p>'}</div><div class="board-social-section"><span class="board-social-label">Discover</span>${discoverGroups.length ? discoverGroups.map((group) => `<div class="board-discover-row"><div><strong>${escapeHtml(group.name)}</strong><small>${escapeHtml(boardGroupVisibilityLabel(group))} · ${group.memberCount || 0} members</small></div>${group.pending ? '<span class="board-pending-label">Applied</span>' : `<button type="button" class="text-button" data-board-join-group="${escapeHtml(group.id)}">${group.visibility === 'circle' ? 'Apply' : 'Join'}</button>`}</div>`).join('') : '<p class="board-sidebar-empty">No open groups yet.</p>'}</div><details class="board-create-group"><summary>＋ Create a group</summary><form id="board-group-form"><label>Group name<input name="name" required maxlength="80" placeholder="e.g. Game builders"></label><label>Description<textarea name="description" maxlength="240" placeholder="What belongs here?"></textarea></label><label>Access<select name="visibility"><option value="public">Open — anyone can join</option><option value="circle">Circle — apply to join</option><option value="private">Private — invited members only</option></select></label><fieldset class="board-share-people"><legend>Initial members</legend>${memberChecks || '<span class="field-note">You can add people when more accounts are available.</span>'}</fieldset><button class="button" type="submit">Create group</button></form></details></div></aside><main class="board-feed-main"><div class="board-feed-heading"><div><span class="editorial-kicker">${selectedGroup ? escapeHtml(boardGroupVisibilityLabel(selectedGroup)) : 'All groups'}</span><h2>${selectedGroup ? escapeHtml(selectedGroup.name) : 'The project feed.'}</h2><p>${selectedGroup ? escapeHtml(selectedGroup.description || 'A focused room for sharing useful work.') : 'Project ideas, snippets, and the next useful conversation.'}</p></div><div class="board-feed-actions"><button type="button" class="board-view-tab is-active">Feed</button><button type="button" class="board-view-tab" data-board-mode="ideas">Ideas</button><button type="button" class="board-view-tab" data-board-mode="library">Projects</button></div></div>${selectedGroup?.isOwner && selectedGroup.pendingApplicants?.length ? `<div class="board-membership-review"><strong>Join requests</strong>${selectedGroup.pendingApplicants.map((applicant) => `<span>${escapeHtml(applicant.username)} <button type="button" class="text-button" data-board-approve-group="${escapeHtml(selectedGroup.id)}" data-board-approve-user="${escapeHtml(applicant.id)}">Approve</button></span>`).join('')}</div>` : ''}${selectedCanPost ? `<form class="board-post-composer" id="board-post-form"><textarea name="body" required maxlength="2000" placeholder="Share a project update, an idea, or a useful snippet…"></textarea><div class="board-post-composer-row"><input name="repoUrl" type="url" placeholder="Optional project link"><button type="button" class="text-button" data-board-toggle-snippet>＋ Code snippet</button><button type="submit" class="button">Post</button></div><textarea name="snippet" class="board-snippet-input" hidden maxlength="1200" placeholder="Paste a small, focused code snippet"></textarea></form>` : selectedGroup ? `<div class="board-join-prompt">${selectedGroup.pending ? 'Your request is waiting for approval.' : 'Join this group to post and reply.'}</div>` : ''}<div class="board-feed-meta"><span>${feedPosts ? `${feedPosts.match(/<article class="board-post/g)?.length || 0} posts` : 'No posts yet'}</span><span>Newest first</span></div><div class="board-feed-list">${feedPosts || '<div class="board-feed-empty"><span class="board-empty-icon">✦</span><h3>Start the conversation.</h3><p>Share a project update, a code snippet, or the next question worth asking.</p></div>'}</div></main></div></section>`;
}

const IDEA_STAGES = [
  { id: 'spark', label: 'Sparks', note: 'Unfiltered thoughts' },
  { id: 'shape', label: 'Shaping', note: 'Clarify the useful part' },
  { id: 'test', label: 'Testing', note: 'Questions and small experiments' },
  { id: 'ready', label: 'Ready', note: 'Clear enough to build' },
];

function boardIdeaCard(idea, groups) {
  const stageIndex = Math.max(0, IDEA_STAGES.findIndex((stage) => stage.id === idea.stage));
  const group = groups.find((item) => item.id === idea.groupId);
  const canEdit = idea.ownerId === currentUser.id;
  return `<article class="idea-card"><div class="idea-card-topline"><span>${group ? escapeHtml(group.name) : 'Personal'}</span><span>${escapeHtml(idea.ownerUsername || currentUser.username)}</span></div><h3>${escapeHtml(idea.title)}</h3>${idea.detail ? `<p>${escapeHtml(idea.detail)}</p>` : ''}<div class="idea-card-actions">${canEdit && stageIndex > 0 ? `<button type="button" class="text-button" data-board-move-idea="${escapeHtml(idea.id)}" data-board-idea-stage="${IDEA_STAGES[stageIndex - 1].id}">← Back</button>` : ''}${canEdit && stageIndex < IDEA_STAGES.length - 1 ? `<button type="button" class="text-button" data-board-move-idea="${escapeHtml(idea.id)}" data-board-idea-stage="${IDEA_STAGES[stageIndex + 1].id}">Next →</button>` : ''}${idea.groupId ? `<button type="button" class="text-button" data-board-discuss-idea="${escapeHtml(idea.id)}">Discuss</button>` : ''}${canEdit ? `<button type="button" class="text-button danger" data-board-archive-idea="${escapeHtml(idea.id)}">Archive</button>` : ''}</div></article>`;
}

function boardIdeasPage() {
  const groups = Array.isArray(boardSocial.groups) ? boardSocial.groups : [];
  const allIdeas = Array.isArray(boardSocial.ideas) ? boardSocial.ideas : [];
  ...
  const counts = Object.fromEntries(IDEA_STAGES.map((stage) => [stage.id, ideas.filter((idea) => (idea.stage || 'spark') === stage.id).length]));
  return `<section class="board-page board-social-page"><div class="board-social-layout${sidebarState}"><aside class="board-social-sidebar${boardFeedSidebarOpen ? '' : ' is-collapsed'}"><button type="button" class="board-sidebar-toggle" data-board-toggle-sidebar aria-expanded="${boardFeedSidebarOpen}" aria-controls="board-ideas-sidebar-content" aria-label="${sidebarButtonLabel}"><span class="board-sidebar-toggle-mark" aria-hidden="true">${boardFeedSidebarOpen ? '‹' : '›'}</span><span class="board-sidebar-toggle-label">${boardFeedSidebarOpen ? 'Hide' : 'Show'} board</span></button><div class="board-social-sidebar-content" id="board-ideas-sidebar-content"><div class="board-social-sidebar-top"><span class="editorial-kicker">Board workspace</span><h2>Three places. One flow.</h2><p>Talk, shape ideas, then organize the projects worth keeping.</p></div><div class="board-social-section board-workspace-links"><span class="board-social-label">Workspace</span><button type="button" class="board-workspace-link" data-board-mode="feed"><span>Feed</span><small>Updates and replies</small></button><button type="button" class="board-workspace-link is-active" data-board-mode="ideas"><span>Idea Lab</span><small>${ideas.length} active ideas</small></button><button type="button" class="board-workspace-link" data-board-mode="library"><span>Projects</span><small>Repos and folders</small></button></div><div class="board-social-section"><span class="board-social-label">Your groups</span>${memberGroups.length ? memberGroups.map((group) => `<button type="button" class="board-workspace-link" data-board-idea-group="${escapeHtml(group.id)}"><span>${escapeHtml(group.name)}</span><small>Open its feed</small></button>`).join('') : '<p class="board-sidebar-empty">Ideas can stay personal until you join a group.</p>'}</div></div></aside><main class="board-feed-main ideas-main"><div class="board-feed-heading"><div><span class="editorial-kicker">Idea Lab</span><h2>Turn sparks into something buildable.</h2><p>Capture quickly, move only the ideas that earn more thought, and archive the rest.</p></div><div class="board-feed-actions"><button type="button" class="board-view-tab" data-board-mode="feed">Feed</button><button type="button" class="board-view-tab is-active">Ideas</button><button type="button" class="board-view-tab" data-board-mode="library">Projects</button></div></div><form id="board-idea-form" class="idea-capture"><label><span>Idea</span><input name="title" required maxlength="100" placeholder="A small idea worth exploring"></label><label class="idea-capture-detail"><span>One-line thought</span><input name="detail" maxlength="500" placeholder="What makes it useful or interesting?"></label><label><span>Where</span><select name="groupId"><option value="">Personal</option>${memberGroups.map((group) => `<option value="${escapeHtml(group.id)}">${escapeHtml(group.name)}</option>`).join('')}</select></label><button type="submit" class="button">Capture</button></form><div class="idea-board">${IDEA_STAGES.map((stage) => `<section class="idea-column"><header><div><span>${escapeHtml(stage.label)}</span><small>${escapeHtml(stage.note)}</small></div><strong>${counts[stage.id]}</strong></header><div class="idea-column-list">${ideas.filter((idea) => (idea.stage || 'spark') === stage.id).map((idea) => boardIdeaCard(idea, groups)).join('') || '<p class="idea-column-empty">Nothing here yet.</p>'}</div></section>`).join('')}</div></main></div></section>`;
}

function boardPage() {
  if (boardMode === 'feed') return boardFeedPage();
  if (boardMode === 'ideas') return boardIdeasPage();
...
async function queueBoardSharedProjects() {
  try {
    const next = await loadBoardProjects();
...
async function queueBoardSocial() {
  try {
    ...
    boardSocial = { groups: Array.isArray(next.groups) ? next.groups : [], posts: Array.isArray(next.posts) ? next.posts : [], replies: Array.isArray(next.replies) ? next.replies : [], ideas: Array.isArray(next.ideas) ? next.ideas : [] };
    if (root.dataset.minimalView === 'board' && ['feed', 'ideas'].includes(boardMode)) renderMinimal('board');
  } catch {
...
function bindBoardIdeaEvents() {
  ...
  document.querySelectorAll('[data-board-idea-group]').forEach((button) => button.addEventListener('click', () => { boardFeedGroupId = button.dataset.boardIdeaGroup; boardMode = 'feed'; renderMinimal('board'); }));
  document.querySelector('#board-idea-form')?.addEventListener('submit', async (event) => { event.preventDefault(); const values = Object.fromEntries(new FormData(event.currentTarget)); try { await boardSocialAction('create-idea', values); await refreshBoardSocial(); } catch (error) { window.alert(error.message); } });
  document.querySelectorAll('[data-board-move-idea]').forEach((button) => button.addEventListener('click', async () => { try { await boardSocialAction('move-idea', { ideaId: button.dataset.boardMoveIdea, stage: button.dataset.boardIdeaStage }); await refreshBoardSocial(); } catch (error) { window.alert(error.message); } }));
  document.querySelectorAll('[data-board-archive-idea]').forEach((button) => button.addEventListener('click', async () => { try { await boardSocialAction('archive-idea', { ideaId: button.dataset.boardArchiveIdea }); await refreshBoardSocial(); } catch (error) { window.alert(error.message); } }));
  document.querySelectorAll('[data-board-discuss-idea]').forEach((button) => button.addEventListener('click', async () => { const idea = boardSocial.ideas.find((item) => item.id === button.dataset.boardDiscussIdea); if (!idea?.groupId) return; try { await boardSocialAction('create-post', { groupId: idea.groupId, body: `Idea: ${idea.title}${idea.detail ? `\n\n${idea.detail}` : ''}` }); boardFeedGroupId = idea.groupId; boardMode = 'feed'; await refreshBoardSocial(); } catch (error) { window.alert(error.message); } }));
}
...
function bindBoardEvents() {
  ...
  document.querySelector('[data-board-link-repo-group]')?.addEventListener('click', async () => {
    const groupId = document.querySelector('[data-board-repo-board-select]')?.value;
    const group = boardSocial.groups.find((item) => item.id === groupId);
    if (!group || !boardRepoViewer) return;
    try {
      await boardSocialAction('create-post', { groupId, body: `Working on ${boardRepoIdentity(boardRepoViewer)?.fullName || boardRepoViewer.name}. Feedback welcome.`, repoUrl: boardRepoViewer.html_url, repoPath: boardRepoViewerContext?.path || '', repoQuery: boardRepoViewerQuery });
      boardFeedGroupId = groupId;
      boardRepoViewerNotice = `Linked to ${group.name}. The group can reply from the feed.`;
      await refreshBoardSocial();
    } catch (error) { window.alert(error.message); }
  });
